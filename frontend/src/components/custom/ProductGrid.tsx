...
const ProductGrid: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-2 gap-6 p-6 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 justify-items-center">
      {products && products.length > 0 ? (
        products.map((product: Product) => (
          <ProductCard key={product._id} product={product} />
        ))
      ) : (
        <p className="text-center text-gray-500">Products unavailable</p>
      )}
    </div>
  );
};
